...
BASE_MODEL=     ### stage1_phase1 checkpoint path
task=comprehension
OUTPUT_BASE_DIR=Time-R1
EXPERIMENT_NAME=${task}/phase2   #all tasks, different alpha

DATA_DIR=${OUTPUT_BASE_DIR}/datasets
OUTPUT_DIR=${OUTPUT_BASE_DIR}/check_points_stage1_phase2
N_GPUS=$(echo $CUDA_VISIBLE_DEVICES | awk -F',' '{print NF}')
ROLLOUT_TP_SIZE=$N_GPUS


python3 -m verl.trainer.main_ppo_s1_p2 \
