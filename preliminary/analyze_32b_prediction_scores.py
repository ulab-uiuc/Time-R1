...
def analyze_32b_predictions(jsonl_file_path):
    """分析DeepSeek 32B蒸馏模型在时间预测任务上的表现"""
    # 初始化总体分数累计器
    total_scores_all = []
    accuracy_scores_all = []
    format_bonuses_all = []
    ...
    for line in tqdm(lines):
        ...
        try:
            ...
            # 32B模型似乎也有reasoning_content字段
            reasoning = message.get('reasoning_content', '') 

            # 提取预测日期
            predicted_date = extract_answer_format(content) # extract_answer_format 会从 content 中提取 <answer> 标签内容
            ...
            # 确保 content.strip() 移除了 <answer> 前后的多余换行和空格
            # 并添加 <|im_end|> 以符合 format_reward_single 的要求
            ...
            # print(f"警告: 无法解析JSON行: {line[:100]}...") # 减少冗余输出
            skipped_count_total += 1
    ...
    if valid_count_total > 0:
        ...
        print("\n===== DeepSeek 32B蒸馏模型 每月得分统计 (2024-07 至 2025-02) =====")
...
if __name__ == "__main__":
    # 修改为32B结果文件的路径
    file_path = "/data/zliu331/temporal_reasoning/TinyZero/preliminary/r1_results/prediction_32b_results.jsonl" 
    
    results = analyze_32b_predictions(file_path)
    
    if results:
        # 可以选择将结果保存到文件
        output_json_path = "/data/zliu331/temporal_reasoning/TinyZero/preliminary/r1_results/32b/prediction_32b_scores_with_monthly.json"
...
#             # 如果无法提取真实日期，跳过该记录
#             if not true_date or not is_valid_date_format(true_date):
#                 # print(f"无法从custom_id '{custom_id}'提取有效的日期格式") # 减少冗余输出
#                 skipped_count += 1
#                 continue
...
#             if not choices:
#                 # print(f"记录中没有模型回答: {custom_id}") # 减少冗余输出
#                 skipped_count += 1
#                 continue
...
#             skipped_count += 1
#         except Exception as e:
#             print(f"处理记录 {custom_id if 'custom_id' in locals() else '未知'} 时出错: {str(e)}")
#             skipped_count += 1
...
#     results = analyze_32b_predictions(file_path)
#     # 可以选择将结果保存到文件
#     # output_json_path = "/data/zliu331/temporal_reasoning/TinyZero/preliminary/r1_results/prediction_32b_scores.json"
#     # if results:
#     #     with open(output_json_path, 'w') as f:
#     #         json.dump(results, f, indent=2)
#     #     print(f"\n得分结果已保存到: {output_json_path}")
