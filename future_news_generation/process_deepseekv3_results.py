...
def extract_news_from_deepseek_response(item):
    """从Deepseek V3 API响应中提取新闻数据"""
    try:
        # 提取响应内容
        custom_id = item.get('custom_id', '')
        response_content = item.get('response', {}).get('body', {}).get('choices', [{}])[0].get('message', {}).get('content', '')
        ...
        if '<answer>' in response_content and '</answer>' in response_content:
            # 提取<answer>标签中的内容
            answer_content = re.search(r'<answer>(.*?)</answer>', response_content, re.DOTALL)
            if answer_content:
                ...
                for i, section in enumerate(news_sections):
                    # 提取标题和摘要
                    headline_match = re.search(r'Headline:?\s*(.*?)\s*Abstract:', section, re.IGNORECASE | re.DOTALL)
...
def process_deepseek_results(input_file, output_dir):
    ...
    print(f"共读取 {len(data)} 条Deepseek V3 API响应")
...
def main():
    # 配置参数
    input_file = "/data/zliu331/temporal_reasoning/TinyZero/future_news_generation/deepseekv3_results/results_v3.jsonl"
    output_dir = "/data/zliu331/temporal_reasoning/TinyZero/future_news_generation/deepseekv3_results"
    count_per_topic = 5
